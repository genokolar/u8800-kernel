...
#define PM_LIB_RPC_VERS					0x00030001 /* 0x00030001 */

/* RPC precedure number for consume notify */
#define PM_CURRENT_CONSUME_NOTIFY_PROC  73
#endif
...
/*delete some lines*/
struct msm_rpc_client *chg_client;
...
static int msm_batt_get_battery_level_ret_func(struct msm_rpc_client *batt_client,
				       void *buf, void *data)
{
	struct msm_batt_get_battery_level_ret_data *data_ptr, *buf_ptr;

	data_ptr = (struct msm_batt_get_battery_level_ret_data *)data;
	buf_ptr = (struct msm_batt_get_battery_level_ret_data *)buf;
...
static u32 msm_batt_get_battery_level(void)
{
	...
	struct msm_batt_get_battery_level_ret_data rep;

	rc = msm_rpc_client_req(chg_client,
			BATTERY_LEVEL_PROC,
			NULL, NULL,
			msm_batt_get_battery_level_ret_func, &rep,
...
/*delete some lines*/
#ifdef CONFIG_HUAWEI_KERNEL 
    chg_client = msm_rpc_register_client("charger", CHG_RPC_PROG,
					CHG_RPC_VER_2_2,
					1, NULL);
    ...
    if (chg_client == NULL) 
    {
		pr_err("%s: FAIL: rpc_register_client. chg_client=NULL\n",
		       __func__);
		return -ENODEV;
	} else if (IS_ERR(chg_client))
    {
		rc = PTR_ERR(chg_client);
		pr_err("%s: ERROR: rpc_register_client: rc = %d\n ",
		       __func__, rc);
		chg_client = NULL;
		return rc;
