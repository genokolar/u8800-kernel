...
#ifndef CONFIG_HUAWEI_FEATURE_PHUDIAG
...
void __diag_smd_send_req(int context)
{
	...
	if (driver->ch ) {
		int r = smd_read_avail(driver->ch);
		phudiagfwd_in_buf_change_busy_state(r,driver->in_busy);
	...
	if (r > USB_MAX_IN_BUF) {
		...
		if (r > 0) {
			...
			if (!buf) {
				printk(KERN_ALERT "Out of diagmem for a9\n");
			} 
			else if(!phudriver->in_buf)
			{
			}
			else 
			{
						...
						if(phudriver->opened)
						{
							if(!phudriver->in_busy && !phudriver->in_buf_busy)
							{
								...
								if (context == SMD_CONTEXT)
									...
									smd_read(driver->ch, buf, r);
								phudiagfwd_write_data_to_in_buf(buf,r);
								phudriver->in_buf_busy = 0;

								#ifdef PHUDIAG_DEBUG
								#endif
								if(driver->usb_connected && !driver->in_busy)
								{
									...
									diag_device_write(buf, MODEM_DATA);
#ifdef DIAG_DEBUG
									printk(KERN_ALERT "writing data to USB,"
											 " pkt length %d \n", r);
						...
						else if(driver->usb_connected && !driver->in_busy)
						{
							APPEND_DEBUG('i');
							if (context == SMD_CONTEXT)
								smd_read_from_cb(driver->ch, buf, r);
...
void __diag_smd_qdsp_send_req(int context)
{
	...
	if (driver->chqdsp ) {
		int r = smd_read_avail(driver->chqdsp);
		phudiagfwd_in_buf_change_busy_state(r,driver->in_busy_qdsp);

		if (r > USB_MAX_IN_BUF) {
			printk(KERN_ALERT "diag dropped num bytes = %d\n", r);
			return;
		}
		if (r > 0) {
			buf = driver->usb_buf_in_qdsp;
			if (!buf) {
				printk(KERN_ALERT "Out of diagmem for q6\n");
			}
			else if(!phudriver->in_buf){
				}
				else {
						...
						if(phudriver->opened)
						{
							if(!phudriver->in_busy && !phudriver->in_buf_busy)
							{
								...
								#ifdef PHUDIAG_DEBUG
								#endif
								if(driver->usb_connected && !driver->in_busy_qdsp)
								{
									driver->usb_write_ptr_qdsp->length = r;
									driver->in_busy_qdsp = 1;
						...
						else if(driver->usb_connected && !driver->in_busy_qdsp)
						{
...
static int diag_smd_probe(struct platform_device *pdev)
{
	...
	phudriver->chqdsp = driver->chqdsp;
	#endif
err:
	return 0;
}
